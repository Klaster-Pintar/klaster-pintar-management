...
@section('title', 'Edit Device - IoT Monitoring')
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h1 class="text-xl lg:text-2xl font-bold text-gray-800 flex items-center gap-2">
                                <i class="fa-solid fa-edit text-yellow-600"></i>
                                Edit IoT Device
                            </h1>
                            <p class="text-sm text-gray-600 mt-1">Update informasi device {{ $device->code }}</p>
                        </div>
                    </div>
                        ...
                        <form id="deviceEditForm" action="{{ route('admin.iot.device-management.update', $device) }}" method="POST" onsubmit="return confirmUpdate(event)">
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="code" value="{{ old('code', $device->code) }}" required
                                    ...
                                    <div>
                                        ...
                                        <select name="cluster_id"
                                            ...
                                            @foreach($clusters as $cluster)
                                                <option value="{{ $cluster->id }}" {{ old('cluster_id', $device->cluster_id) == $cluster->id ? 'selected' : '' }}>
                                                    {{ $cluster->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div>
                                    ...
                                    <textarea name="description" rows="3"
                                        ...
                                        placeholder="Deskripsi detail tentang device ini...">{{ old('description', $device->description) }}</textarea>
                                    @error('description')
                                        <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <select name="hardware_status" required
                                            ...
                                            <option value="Active" {{ old('hardware_status', $device->hardware_status) == 'Active' ? 'selected' : '' }}>Active</option>
                                            <option value="Inactive" {{ old('hardware_status', $device->hardware_status) == 'Inactive' ? 'selected' : '' }}>Inactive</option>
                                            <option value="Rusak" {{ old('hardware_status', $device->hardware_status) == 'Rusak' ? 'selected' : '' }}>Rusak</option>
                                        </select>
                                    ...
                                    <div>
                                        ...
                                        <select name="network_status" required
                                            ...
                                            <option value="Connected" {{ old('network_status', $device->network_status) == 'Connected' ? 'selected' : '' }}>Connected
                                            </option>
                                            <option value="Not Connected" {{ old('network_status', $device->network_status) == 'Not Connected' ? 'selected' : '' }}>Not Connected
                                            </option>
                                        </select>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <input type="number" name="signal_strength"
                                            value="{{ old('signal_strength', $device->signal_strength) }}" min="0" max="100"
                            ...
                            <div class="flex gap-3 mt-6 pt-6 border-t">
                                <button type="submit"
                                    class="px-6 py-2 bg-gradient-to-r from-yellow-500 to-yellow-600 text-white rounded-lg hover:shadow-lg transition font-medium">
                                    <i class="fa-solid fa-save mr-1"></i> Update Device
                                </button>
                                <a href="{{ route('admin.iot.device-management.index') }}"
